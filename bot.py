...
async def start(message: types.Message):
    start_buttons = ['Скидки АТБ Харьков']#, 'Ekaterinburg'
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.add(*start_buttons)
    await message.answer('Please select a Button', reply_markup=keyboard)
...
async def moscow_city(message: types.Message):
    ...
    await send_data(city_code='2398',chat_id=chat_id)#city_code='2398',
